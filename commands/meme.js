    ...
    async execute(interaction) {
        ...
        async function redditMeme() {
            await fetch('https://www.reddit.com/r/memes/random/.json').then(async res => {
                ...
                return interaction.reply({ embeds: [embed.setTitle(title).setColor("#2F3136").setImage(url).setURL(url).setColor("Random").setFooter({ text: author })] });
            });
        }
        ...
        async function giphyMeme() {
            await fetch('https://api.giphy.com/v1/gifs/random?api_key=lYBciWOvaOhVPNzGy4J8mUXNpC53YNbz&tag=&rating=g').then(async res => {
                ...
                return interaction.reply({
                    embeds: [embed.setTitle(`${title}`).setImage(`${url}`).setColor("#2F3136").setURL(link).setColor("Random").setFooter({ text: `Command requested by ${interaction.member.user.username}, Running DJS V${version}` }).setTimestamp()],
                });
            });
        }
